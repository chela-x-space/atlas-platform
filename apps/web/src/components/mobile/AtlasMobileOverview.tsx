...
export function AtlasMobileOverview(){
  ...
  return <div className="atlas-mobile">
    ...
    {moreOpen?<div className="atlas-mobile-more" id="atlas-mobile-more"><div><strong>More ATLAS services</strong><button type="button" onClick={()=>setMoreOpen(false)} aria-label="Close menu">×</button></div><Link href="/app/reports">Reports Center</Link><Link href="/app/risk">Global Risk</Link><Link href="/app/breaking">Breaking News</Link><Link href="/app/ai">AI Technology Radar</Link><Link href="/app/metrics">Global Metrics</Link><Link href="/app/timeline">Global Timeline</Link><Link href="/app/news">News Center</Link><Link href="/app/weather">Weather · configuration required</Link><Link href="/app/markets">Markets · integration pending</Link><a href="/app?desktop=1">Full dashboard</a></div>:null}
  </div>;
}
